...
def get_svg(origin_x: int, origin_y: int, x2: int, y2: int, magnification: int=1):
    ...
    patch_index_y = grid_aligned_origin_y
    for y in range(grid_aligned_origin_y, y2 + 1, patch_size):
        patch_index_x = grid_aligned_origin_x
...
def mouse_handler(e: MouseEventArguments):
    ...
    if e.type == 'mousedown':
        ...
        update_user_view_state(mouse_down=(e.type == 'mousemove'), pan_start=(
            e.image_x, e.image_y), current_image_corner=(x, y, magnification))
        e.sender.set_content(
            get_svg(x, y, x+viewport_width, y+viewport_height, magnification))
    elif e.type == 'mouseleave':
        update_user_view_state(mouse_down=False)


def wheel_handler(e: GenericEventArguments):
    # print(f"{e=}, {type(e)=} {e.args['deltaY']=}")
    x, y, magnification = app.storage.user.get('current_image_corner', (5000, 5000, 1))
    if e.args['deltaY']<0:
...
async def main_page():
    ...
    viewer = ui.interactive_image(
        content=get_svg(x, y, viewport_width, viewport_height, magnification), 
