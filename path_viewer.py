...
import signal
process_pool_executor = concurrent.futures.ProcessPoolExecutor(max_workers=20)
...
def save_image(method_image_path:Path, patch):
    method_image_path.parent.mkdir(parents=True, exist_ok=True)
    patch.save(method_image_path)
...
async def get_patch(x: int, y: int, magnification: int):
    try:
        ...
        if magnification > 1: 
            ...
            method_image_path = image_index_root / image_key / 'original' / f'{magnification}' / f'{x}_{y}.png'
            if method_image_path.exists():
                ...
                buff.seek(0)
                #save image to disk for caching
                process_pool_executor.submit(save_image, method_image_path, patch)
...
@app.get('/x/{method}/{x}/{y}/{magnification}',
         # Set what the media type will be in the autogenerated OpenAPI specification.
         # fastapi.tiangolo.com/advanced/additional-responses/#additional-media-types-for-the-main-response
         responses={
             200: {
...
async def get_processed_patch(method: str, x: int, y: int, magnification: int):
    try:
        image_key = app.storage.user.get('image_key', 'C-17-036-2021-07-15_215134__ndpi')
        ...
        if magnification > 1: 
            method_image_path = image_index_root / image_key / method / f'{magnification}' / f'{x}_{y}.png'
            if method_image_path.exists():
                with method_image_path.open('rb') as f:
                    buff = io.BytesIO()
                    buff.write(f.read())
                ...
                patch = combine_patches(image_index_root, image_key, method, (x, y), magnification, (patch_size, patch_size))
                buff = io.BytesIO()
                patch.save(buff, format='PNG')
                buff.seek(0)
                #save image to disk for caching
    ...
    for y in range(grid_aligned_origin_y, y2 + 1, patch_size):
        for x in range(grid_aligned_origin_x, x2 + 1, patch_size):
            ...
            for m in methods:
                flag = app.storage.user.get(f'draw_{m}', False)
                if flag:
                    svg += load_p_patch_svg(m, x, y, origin_x, origin_y, magnification)
...
def mouse_handler(e: MouseEventArguments):
    ...
    if e.type == 'mousedown':
        ...
        e.sender.set_content(get_svg(x, y, x+viewport_width, y+viewport_height, magnification))
...
def update_pca_draw(flag: bool, viewer):
    ...
    viewer.set_content(get_svg(x, y, x+viewport_width, y+viewport_height, magnification))
...
async def main_page():
    ...
    viewer = ui.interactive_image(
        content=get_svg(x, y, x+viewport_width, y+viewport_height, magnification), 
